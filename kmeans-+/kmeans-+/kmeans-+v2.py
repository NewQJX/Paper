...
if __name__ == "__main__":
    ...
    # while success <= 2:  
        ...
        print("找到的maxGain为:",maxGain)
        print("找到的maxGainIndex为:",max_Gain_index)
        ...
        #改变簇中心
        current_cluster_centers[min_Cost_index] = X[kmeans.labels_ == max_Gain_index][np.random.RandomState(555).randint(0, len(X[kmeans.labels_ == max_Gain_index]))]
        #重新聚类
        kmeans1 = KMeans(n_clusters=10, init=current_cluster_centers, max_iter=1)
        kmeans1.fit(X)
        new_SSEDM = kmeans1.inertia_
        current_cluster_centers = kmeans1.cluster_centers_
